...
class Migration {
	...
	private function convert_map_defaults($settings) {
		...
		$keys = array(
			...
			'layersCollapsed'              => 'defaults_layer_controlbox',
			'filtersCollapsed'             => 'defaults_layer_mlm_filter_controlbox',
		...
		if (isset($settings['minimap_status'])) {
			$converted['minimapMinimized'] = ($settings['minimap_status'] == 'collapsed');
		}
	...
	public function copy_icons($settings) {
		...
		if (($handle = @opendir($icons_dir)) !== false) {
			while (($file = readdir($handle)) !== false) {
				if (!is_dir($icons_dir . $file) && in_array(substr($file, -4), $allowed)) {
