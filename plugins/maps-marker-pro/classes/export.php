...
class Export {
			...
			foreach ($markers as $marker) {
				$geojson['features'][] = array(
					...
					'properties' => array(
						...
						'popup' => apply_filters('mmp_popup', $l10n->__($marker->popup, "Marker (ID {$marker->id}) popuptext")),
						'link' => ($marker->link),
	...
	private function kml($markers) {
		...
		foreach ($markers as $marker) {
			...
			$this->add_cdata('description', apply_filters('mmp_popup', $l10n->__($marker->popup, "Marker (ID {$marker->id}) popuptext")), $placemark);
	...
	private function georss($map, $markers) {
		...
		foreach ($markers as $marker) {
			...
			$this->add_cdata('description', apply_filters('mmp_popup', $l10n->__($marker->popup, "Marker (ID {$marker->id}) popuptext")), $item);
	...
	private function atom($map, $markers) {
		...
		foreach ($markers as $marker) {
			...
			$this->add_cdata('content', apply_filters('mmp_popup', $l10n->__($marker->popup, "Marker (ID {$marker->id}) popuptext")), $entry);
