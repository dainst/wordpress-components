...
class Menu_Settings extends Menu {
	...
	public function save_settings() {
		...
		$settings = $mmp_settings->validate_settings($settings, false);
			...
			<form id="settings" method="POST">
				...
				<div class="mmp-settings-wrap">
					...
					<div class="mmp-settings-nav">
						...
						<div class="mmp-settings-nav-group">
							<span><?= esc_html__('Augmented-Reality', 'mmp') ?></span>
							<ul>
								<li id="ar_layar_link" class="mmp-tablink">layar</li>
							</ul>
						</div>
						<div class="mmp-settings-nav-group">
					...
					<div class="mmp-settings-tabs">
						...
						<div id="ar_layar_tab" class="mmp-settings-tab">
							<h2>layar</h2>
							<p>
								<?= sprintf(esc_html__('Markers can also be displayed via Augmented-Reality technology powered by %1$s on mobile devices. Sample screenshots:', 'mmp'), '<a href="https://www.layar.com" target="_blank">layar</a>') ?>
							</p>
							<p>
								<img src="<?= plugins_url('images/options/layar-example.jpg', __DIR__) ?>" />
							</p>
							<p>
								<?= sprintf(esc_html__('For instructions on how to connect your maps to layar, please visit %1$s.', 'mmp'), '<a href="https://www.mapsmarker.com/layar/" target="_blank">https://www.mapsmarker.com/layar/</a>') ?>
							</p>
							<p>
								<?= esc_html__('Layar API endpoint', 'mmp') ?>: <strong><?= $api->link('/layar/') ?></strong>
							</p>
							<div class="mmp-settings-setting">
								<div class="mmp-settings-desc"><?= esc_html__('Radius', 'mmp') ?></div>
								<div class="mmp-settings-input">
									<input type="text" name="layarRadius" value="<?= $settings['layarRadius'] ?>" /><br />
									<span class="mmp-settings-footnote">
										<?= esc_html__('Instead of providing a fixed search range, you can also return a radius value in the response to assure that certain POIs are presented to the user. This can be done by emptying the default value of the search range filter on the publishing site on layar.com. The unit is meters.', 'mmp') ?>
									</span>
								</div>
							</div>
							<div class="mmp-settings-setting">
								<div class="mmp-settings-desc"><?= esc_html__('Distance unit', 'mmp') ?></div>
								<div class="mmp-settings-input">
									<ul>
										<li><label><input type="radio" name="layarUnits" value="km" <?= $this->checked($settings['layarUnits'], 'km') ?> /> <?= esc_html__('Metric (km)', 'mmp') ?></label></li>
										<li><label><input type="radio" name="layarUnits" value="mile" <?= $this->checked($settings['layarUnits'], 'mile') ?> /> <?= esc_html__('Imperial (miles)', 'mmp') ?></label></li>
									</ul>
								</div>
							</div>
							<div class="mmp-settings-setting">
								<div class="mmp-settings-desc"><?= esc_html__('POI icons', 'mmp') ?></div>
								<div class="mmp-settings-input">
									<ul>
										<li><label><input type="radio" name="layarIcons" value="mapsmarker" <?= $this->checked($settings['layarIcons'], 'mapsmarker') ?> /> <?= esc_html__('Use distinct marker icons', 'mmp') ?></label></li>
										<li><label><input type="radio" name="layarIcons" value="layar_standard" <?= $this->checked($settings['layarIcons'], 'layar_standard') ?> /> <?= esc_html__('Use standard POI icons with custom colors from dev.layar.com', 'mmp') ?></label></li>
										<li><label><input type="radio" name="layarIcons" value="layar_custom" <?= $this->checked($settings['layarIcons'], 'layar_custom') ?> /> <?= esc_html__('Use custom POI icons with POI type 1 from dev.layar.com', 'mmp') ?></label></li>
									</ul>
								</div>
							</div>
							<div class="mmp-settings-setting">
								<div class="mmp-settings-desc">refreshInterval</div>
								<div class="mmp-settings-input">
									<input type="text" name="layarRefreshInterval" placeholder="300" value="<?= $settings['layarRefreshInterval'] ?>" /><br />
									<span class="mmp-settings-footnote">
										<?= sprintf(esc_html__('This tells the layar client how many seconds to wait until the next getPOIs request is made. By default, it is %1$s. The minimum value is %2$s.', 'mmp'), '300', '10') ?>
									</span>
								</div>
							</div>
							<div class="mmp-settings-setting">
								<div class="mmp-settings-desc">refreshDistance</div>
								<div class="mmp-settings-input">
									<input type="text" name="layarRefreshDistance" placeholder="100" value="<?= $settings['layarRefreshDistance'] ?>" /><br />
									<span class="mmp-settings-footnote">
										<?= sprintf(esc_html__('This tells the layar client to generate a new getPOIs request if the user has moved by more than the specified distance (in meters). By default, it is %1$s. The minimum value is %2$s.', 'mmp'), '100', '3') ?>
									</span>
								</div>
							</div>
							<div class="mmp-settings-setting">
								<div class="mmp-settings-desc"><?= esc_html__('Footnote', 'mmp') ?></div>
								<div class="mmp-settings-input">
									<input type="text" name="layarFootnote" placeholder="powered by www.mapsmarker.com" value="<?= $settings['layarFootnote'] ?>" /><br />
									<span class="mmp-settings-footnote">
										<?= esc_html__('Text that gets added to each POI description.', 'mmp') ?>
									</span>
								</div>
							</div>
						</div>
