...
margin-bottom: 10px;",
			...
			$this->presets = array(	// Default out of the box
				'1' => array(
					'preset_name' => 'Preset 1: Out of the box (default)'
				),
				// Author's favorite
				'2' => array(
				...
				// Flickr style
				'3' => array(
					'preset_name' => 'Preset 3: Flickr style',
					'thumbs_spacing' => 8,
					'row_height' => 230,
					'height_deviation' => 80,
					'overlay' => 'off',
...
text-align:left;",
				...
				// G+ style
				'4' => array(
					'preset_name' => 'Preset 4: Google+ style',
					'thumbs_spacing' => 6,
					'row_height' => 280,
					'height_deviation' => 55,
					'overlay' => 'off',
				...
				'5' => array(
					'preset_name' => 'Preset 5: Fixed height, no fancy',
					'thumbs_spacing' => 5,
					'row_height' => 175,
					'height_deviation' => 0,
					'caption' => 'off',
				...
				// Artistic zen
				'6' => array(
					'preset_name' => 'Preset 6: Artistic-zen',
					'thumbs_spacing' => 0,
					'row_height' => 240,
					'height_deviation' => 60,
					'overlay' => 'others',
				...
				'7' => array(
					'preset_name' => 'Preset 7: Color magic fancy style',
					'thumbs_spacing' => 0,
					'row_height' => 250,
...
text-transform:uppercase;",
				...
				// No links big images
				'8' => array(
					'preset_name' => 'Preset 8: Big images no click',
					'thumbs_spacing' => 1,
					'row_height' => 350,
					'height_deviation' => 50,
					'overlay' => 'others',
				...
				// Focus on the text
				'9' => array(
					'preset_name' => 'Preset 9: Focus on the text',
					'thumbs_spacing' => 3,
					'row_height' => 250,
					'height_deviation' => 50,
					'caption' => 'mixed',
...
text-align:left;",
				...
				// Magnifier blur
				'11' => array(
					'preset_name' => 'Preset 11: Magnifier blur',
					'caption' => 'off',
					'overlay' => 'hovered',
					'overlay_icon' => 'on',
				...
				// Orton effect
				'13' => array(
					'preset_name' => 'Preset 13: Orton effect',
					'overlay' => 'hovered',
				...
				'14' => array(
					'preset_name' => 'Preset 14: Animated border and glow',
					'thumbs_spacing' => 0,
					'overlay' => 'others',
					'inner_shadow' => "0 0 30px black",
				...
				// Borders and shadow
				'15' => array(
					'preset_name' => 'Preset 15: Borders and shadow',
					'thumbs_spacing' => 15,
					'caption' => 'off',
					'overlay' => 'others',
					'overlay_color' => 'black',
				...
				// Facebok inspired
				'16' => array(
					'preset_name' => 'Preset 16: Facebok inspired',
					'animation_speed' => 400,
					'caption' => 'slide',
					'overlay' => 'hovered',
					'mobile_overlay' => 'off',
				...
				// Vertical center
				'17' => array(
					'preset_name' => 'Preset 17: Vertical center',
					'caption' => 'fade',
					'caption_opacity' => 1,
				...
				// Vertical creative
				'18' => array(
					'preset_name' => 'Preset 18: Vertical creative',
					'caption' => 'slide',
					'caption_opacity' => 1,
				...
				'19' => array(
					'preset_name' => 'Preset 19: Caption fun, gray background',
					'caption' => 'mixed',
				...
				'20' => array(
					'preset_name' => 'Preset 20: Caption below the thumbs',
					'caption' => 'below',
					'caption_opacity' => 1,
					'caption_bg_color' => 'transparent',
					'caption_align' => 'center',
		...
		function jig_init(){
			if($this->settings['take_over_gallery'] === 'yes'){
				...
				add_filter(
					'the_content',
					array($this, 'take_over_gutenberg_gallery_in_content'),
					-1000
				);
			}
			...
			if (get_user_option('rich_editing') == 'true') {
				...
				add_filter('mce_buttons', array($this, 'register_jig_shortcode_editor'), -1000);
		...
		function take_over_gutenberg_gallery_in_content($content){
			$taken_over = preg_replace(
					...
					if(!empty($provider) && !empty($content_id)){
						...
						switch ($provider) {
							...
							case 'FB':
								...
								$photos_url = 'https://graph.facebook.com/v3.2/'.$content_id.'?fields=name,images&access_token='.$token;
...
jQuery('#jigPresetSelect').val('c".$custom_preset_id."');});</script>";
		...
		function jig_print_fb_authed(){
			...
			$output .= '<div id="jigFbAuthOtherUserPanel">
							<ol>
								...
								<li>'.__('Send him/her this link', 'jig_td').':<br /><input type="text" id="jigFbOtherUserLink" value="" data-force="https://www.facebook.com/v3.2/dialog/oauth?client_id='.trim($this->settings['fb_app_id']).'&scope=user_photos,user_posts,user_videos,pages_show_list' . '&redirect_uri='.urlencode(plugins_url('fb-auth-other-user.php', __FILE__)).'" /></li>
		...
		*/
		...
		function jig_take_over_nextgen_ngg_shortcode($attr) {
			$output = '';
			if(function_exists('get_jig')){
				$parameters = array();
				if(!empty($attr['src'])){
		...
		function jig_init_shortcode($atts){
			...
			if(in_array('wp_head', $wp_current_filter) === true){
				return;
			}
...
*/
				...
				case 'facebook':
					...
					if($facebook_album == 'overview' || $facebook_album == 'overview_only_albums'){
						if($fb_lightbox_album == 'yes'){
							if(empty($facebook_album_multi) || (!empty($facebook_album_multi) && $fb_album_exclude == 'yes')){ // Preventing showing everything at once with lightbox albums.
								$preliminary_count_check_albums_url = 'https://graph.facebook.com/v3.2/'.$facebook_id.'?fields=albums.limit('.$limit.').fields(count)&access_token='.$token;
							...
							$albums_url = 'https://graph.facebook.com/v3.2/'.$facebook_id.'?fields=albums.limit('.$limit.').fields(id,cover_photo,link,count,name,description,type,photos.limit('.$auto_limit.').fields(images,source,height,width,name))&access_token='.$token;
						}else{
							$albums_url = 'https://graph.facebook.com/v3.2/'.$facebook_id.'?fields=albums.limit('.$limit.').fields(id,cover_photo,link,count,name,description,type,photos.limit(1).fields(images))&access_token='.$token;
						}
						...
						if(!empty($facebook_album_multi) && $facebook_album_multi[0] == 'latestone'){
							...
							$albums_url = 'https://graph.facebook.com/v3.2/'.$facebook_id.'?fields=albums.limit(5).fields(id,cover_photo,link,count,name,description,type,photos.limit('.$limit.').fields(images,source,height,width,name))&access_token='.$token;
									...
									if(!empty($album->count) && !empty($album->link) && !empty($album->photos->data)){
										if($fb_lightbox_album == 'no' && $fb_actual_cover_photo == 'yes' && !empty($album->cover_photo)){
											...
											if(is_string($album->cover_photo)){
												$fb_cover_photo_url = 'https://graph.facebook.com/v3.2/'.$album->cover_photo.'?fields=images&access_token='.$token;
											}elseif(!empty($album->cover_photo->id)){
												$fb_cover_photo_url = 'https://graph.facebook.com/v3.2/'.$album->cover_photo->id.'?fields=images&access_token='.$token;
											}
					...
					if($facebook_album == 'feed'){
						...
						$feed_url = 'https://graph.facebook.com/v3.2/'.$facebook_id.'/feed?fields=picture,caption,description,message,object_id'.$limit_parameter.'&access_token='.$token;
						...
						if(!empty($photos)){
							...
							foreach ($photos as &$feed_post) {
								if(!(isset($feed_post->picture) && isset($feed_post->object_id))){
									...
									$photo_object_url = 'https://graph.facebook.com/v3.2/'.$feed_post->object_id.'?fields=images,source,height,width&access_token='.$token;
					...
					if($facebook_album == 'videos'){
						$video_url = 'https://graph.facebook.com/v3.2/'.$facebook_id.'/videos/uploaded?fields=source,format,title,description'.$limit_parameter.'&access_token='.$token;
					...
					if($facebook_album !== 'overview' && $facebook_album !== 'overview_only_albums' && $facebook_album !== 'feed' && $facebook_album !== 'videos'){
						...
						$photos_url = "https://graph.facebook.com/v3.2/".$facebook_album."/photos?fields=source,height,width,name".($facebook_image_size == 'normal' ? '' : ',images').$limit_parameter.'&access_token='.$token;
						...
						if(!empty($photos) && empty($photos['message'])){
							...
							if(($facebook_description == 'yes' || $facebook_description == 'above') && empty($facebook_description_displayed_already)){
								$facebook_album_url = "https://graph.facebook.com/v3.2/".$facebook_album."?fields=description".'&access_token='.$token;
								...
								foreach ($collections[$flickr_collection]['set'] as $single_set) {
									...
									if($flickr_count == 'yes'){
										$description_fragments = array(
											($single_set['photos']+$single_set['videos']).' '._n('Photo', 'Photos',
		...
		function jig_fb_auth() {
			...
			if (empty($_REQUEST["code"])) {
				...
				$dialog_url = "https://www.facebook.com/v3.2/dialog/oauth?client_id=" 
					.$app_id."&scope=user_photos,user_posts,user_videos,pages_show_list,manage_pages"
		...
		function get_fb_user_info($params, $tries = 0){
			$graph_url = "https://graph.facebook.com/v3.2/me?fields=name,picture,accounts,albums.limit(1000).fields(count,link)&access_token=" 
					.$params->access_token;
			...
			$tries++;
			if($tries < 10 && !empty($user->error->code) && $user->error->code == 2){
		...
		function fb_verify_album_data_core($token, $owner, $fb_entity, $type){
			$albums_url = "https://graph.facebook.com/v3.2/"
					.$fb_entity
			...
			if(!empty($albums) && empty($albums['message'])){
				...
				if($found > 0){
					$entity_url = "https://graph.facebook.com/v3.2/".$fb_entity.'?fields=name,picture&access_token='.$token;
				...
				$main_url = "https://graph.facebook.com/v3.2/".$fb_entity.'?access_token='.$token;
		...
		function jig_get_fb_albums(){
			...
			$albums_url = "https://graph.facebook.com/v3.2/".$user_id.'?fields=albums.limit(1000).fields(id,cover_photo,link,count,name,description,type,photos.limit(1).fields(images))&access_token='.$token;
			...
			if(!empty($albums)){
				...
				$videos_url = "https://graph.facebook.com/v3.2/".$user_id.'/videos/uploaded?fields=format.fields(picture)&access_token='.$token;
